...
# Inserção de UMA faixa de linhas (para progresso incremental). Sem caixa aqui.
_INSERT_DC_RANGE = """
with ins as (
  insert into ativo.direito_creditorio (
    data_aquisicao, fundo_id, classe_id, lote_id, cedente_id, sacado_id,
  ...
  join ativo.cedente ced on ced.documento = s.cedente_documento
  join ativo.sacado  sac on sac.documento = s.sacado_documento
  where s.lote_id = $1 and s.valido and s.linha_numero >= $2 and s.linha_numero < $3
  on conflict (fundo_id, identificador_externo, data_aquisicao) do nothing
  returning 1
)
select count(*) from ins
...
# Débito de caixa da aquisição, agregado por (fundo, classe, data) do lote inteiro.
_CAIXA_LOTE = """
insert into ativo.movimento_caixa (fundo_id, classe_id, data, tipo, valor, descricao, lote_id)
select fundo_id, classe_id, data_aquisicao, 'AQUISICAO_DC',
       -sum(valor_aquisicao), 'Aquisição de recebíveis (lote)', $1
...
where lote_id = $1
group by fundo_id, classe_id, data_aquisicao
"""


async def importar_boleta_stream(nome: str, conteudo: bytes, user: dict, chunk_size: int = 5000):
    """Versão em streaming: processa em lotes e emite eventos de progresso.

    Emite dicts (serializados como NDJSON pelo router):
      {tipo: status,   etapa: recebido|validando|validado|staging|contrapartes, ...}
      {tipo: progresso, processadas, total, inseridas}
      {tipo: resultado, ...campos do ImportacaoResultado}
    """
    yield {"tipo": "status", "etapa": "recebido"}

    df = validacao.ler_arquivo(nome, conteudo)  # pode levantar ValueError
    arquivo_hash = hashlib.sha256(conteudo).hexdigest()
    total = df.height
    yield {"tipo": "status", "etapa": "validando", "total": total}
    ...
    async with pool().acquire() as con:
        ...
        yield {"tipo": "status", "etapa": "validado", "total": total,
               "validas": validos, "rejeitadas": rejeitadas}
        ...
        async with con.transaction():
            fundo_id = None
            if cnpj_lote:
                fundo_id = await con.fetchval("select id from fund.fundo where cnpj = $1", cnpj_lote)
            ...
            lote_id = await con.fetchval(
                """insert into ativo.lote_importacao
                   ...
                   values ($1, $2, $3, $4::date, 'VALIDANDO', $5, $6) returning id""",
                fundo_id, nome, arquivo_hash, data_ref, total, user["id"])

            registros = [(lote_id, *linha) for linha in df.select(_STAGING_COLS).rows()]
            await con.copy_records_to_table(
                "boleta_staging", schema_name="ativo",
                columns=["lote_id", *_STAGING_COLS], records=registros)
            yield {"tipo": "status", "etapa": "staging", "total": total}
            ...
            await con.execute(
                """insert into ativo.sacado (documento, nome)
                   ...
                   from ativo.boleta_staging where lote_id = $1 and valido
                   on conflict (documento) do nothing""", lote_id)
            yield {"tipo": "status", "etapa": "contrapartes", "total": total}

            # inserção em lotes por faixa de linha (linha_numero começa em 2)
            inseridos = 0
            start = 2
            fim = total + 2
            while start < fim:
                hi = start + chunk_size
                n = await con.fetchval(_INSERT_DC_RANGE, lote_id, start, hi)
                inseridos += int(n or 0)
                yield {"tipo": "progresso", "processadas": min(hi - 2, total),
                       "total": total, "inseridas": inseridos}
                start = hi

            await con.execute(_CAIXA_LOTE, lote_id)
            await con.execute(
                """update ativo.lote_importacao
                     set status = 'PROCESSADO', linhas_aceitas = $2,
                         linhas_rejeitadas = $3, concluido_em = now() where id = $1""",
                lote_id, validos, rejeitadas)
            await audit.registrar(
                con, acao="BOLETA_IMPORTADA", usuario_id=str(user["id"]),
                usuario_email=user["email"], entidade="ativo.lote_importacao",
                entidade_id=str(lote_id),
                depois={"arquivo": nome, "total": total, "aceitas": validos,
                        "rejeitadas": rejeitadas, "inseridos": inseridos})

        amostra = [
            {"linha": int(r[0]), "motivo": r[1]}
            for r in df.filter(pl.col("valido").not_())
                       .select(["linha_numero", "motivo_rejeicao"]).head(20).rows()
        ]

    yield {"tipo": "resultado", "lote_id": str(lote_id), "arquivo": nome, "status": "PROCESSADO",
           "linhas_total": total, "linhas_aceitas": validos, "linhas_rejeitadas": rejeitadas,
           "registros_inseridos": inseridos, "duplicados_ignorados": validos - inseridos,
